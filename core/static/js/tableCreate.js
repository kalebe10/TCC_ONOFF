...
const switchAtiva = (func, codigo, value) => {
    if (value) {
        check = "checked";
    } else {
        check = "";
    ...
    html = `<td  style='cursor:pointer'>
                     <div class="custom-control custom-switch">
                         <input type="checkbox" onclick='${func}("${codigo}","switch${codigo}")' class="custom-control-input" id="switch${codigo}" ${check}>
    ...
    Object.keys(tbody).map((key) => {
        ...
        tbodyHtml += deleta
            ? `<td><i style='cursor:pointer' class="fas fa-minus-circle"  onclick='${
                  apoio.excluir
              }("${data[apoio.busca]}","${data[apoio?.name] || data[apoio.busca]}")'></i></td>`
            : "";
        tbodyHtml += "</tr>";
    });
    tbodyHtml += `</tbody>`;
